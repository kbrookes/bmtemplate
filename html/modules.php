...
//Simplified Joostrap
function modChrome_joostrap_style_simplified($module, $params, $attribs) 
{
	static $modulescount;
	global $modules;
	...
	if (!empty ($module->content)) {
		...
		echo '<'.$params->get('module_tag', 'div').' id="'.$params->get('header_class').'" class="module'. $modClassSfx .'">';
		if ($module->content) {
			if ($module->showtitle) {
				echo '<'.htmlspecialchars($params->get('header_tag', 'h3')).' class="'.$params->get('header_class').' moduleTitle">'. $module->title .'</'.htmlspecialchars($params->get('header_tag', 'h3')).'>';
			}
			echo $module->content;
		}
		echo '</'.$params->get('module_tag', 'div').'>';
